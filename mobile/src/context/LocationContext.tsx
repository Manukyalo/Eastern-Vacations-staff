import React, { createContext, useContext, useState, useEffect } from 'react';
import { useAuth } from './AuthContext';
import LocationEngine from '../engine/LocationEngine';
import { db } from '@/firebase';
import { doc, setDoc, serverTimestamp } from 'firebase/firestore';

interface LocationUpdate {
  latitude: number;
  longitude: number;
  heading: number;
  speed: number;
  accuracy: number;
  bookingId: string;
  isOnline: boolean;
  batteryLevel: number;
  lastUpdated: Date;
}

interface LocationContextType {
  currentLocation: LocationUpdate | null;
  isTracking: boolean;
  role: string | null;
}

const LocationContext = createContext<LocationContextType | undefined>(undefined);

export const LocationProvider = ({ children }: { children: React.ReactNode }) => {
  const { currentUser, role, isApproved } = useAuth();
  const [currentLocation, setCurrentLocation] = useState<LocationUpdate | null>(null);
  const [isTracking, setIsTracking] = useState(false);

  useEffect(() => {
    // Track location only for approved personnel
    const shouldTrack = role && ['safari_driver', 'driver', 'porter', 'tour_guide'].includes(role);
    
    if (shouldTrack && isApproved && currentUser) {
      LocationEngine.start(currentUser.uid, 'active', async (update) => {
        setCurrentLocation(update);
        setIsTracking(true);

        // Sync to Firestore for Admin Live Tracking
        try {
          await setDoc(doc(db, 'driverLocations', currentUser.uid), {
            driverId: currentUser.uid,
            role,
            latitude: update.latitude,
            longitude: update.longitude,
            heading: update.heading || 0,
            speed: update.speed || 0,
            batteryLevel: update.batteryLevel || 100,
            isOnline: true,
            lastUpdated: serverTimestamp()
          }, { merge: true });
        } catch (error) {
          console.error('[LocationContext] Location Sync Error:', error);
        }
      });

      return () => {
        LocationEngine.stop();
        // Set offline in Firestore when unmounting or stopping tracking
        if (currentUser) {
          setDoc(doc(db, 'driverLocations', currentUser.uid), {
            isOnline: false,
            lastUpdated: serverTimestamp()
          }, { merge: true }).catch(err => console.error('[LocationContext] Offline set failed', err));
        }
        setIsTracking(false);
      };
    }
  }, [currentUser, role, isApproved]);

  const value = {
    currentLocation,
    isTracking,
    role
  };

  return (
    <LocationContext.Provider value={value}>
      {children}
    </LocationContext.Provider>
  );
};

export const useLocation = () => {
  const context = useContext(LocationContext);
  if (!context) {
    throw new Error('useLocation must be used within a LocationProvider');
  }
  return context;
};
